...
'''
if "# §3.B.i Context help / hover descriptions" not in text:
    if marker not in text:
        raise SystemExit("Tools layout marker not found")
    text = text.replace(marker, marker + tooltip_block, 1)
    changed = True
...
for cleanup in (
    Path(".github/workflows/apply-main-tooltips.yml"),
    Path(".pr3-readability-trigger"),
):
    if cleanup.exists():
        cleanup.unlink()
