...
if old not in text:
    raise SystemExit("Route result raw block not found")
text = text.replace(old, new, 1)

# Wi-Fi raw netsh -> hidden details.
old = '''            raw_group = QGroupBox("Raw Wi-Fi Command Output")\n            raw_layout = QVBoxLayout()\n            self.wifi_raw_box = QTextEdit()\n            self.wifi_raw_box.setReadOnly(True)\n            self.wifi_raw_box.setMinimumHeight(150)\n            self.wifi_raw_box.setLineWrapMode(QTextEdit.WidgetWidth)\n            raw_layout.addWidget(self.wifi_raw_box)\n            raw_group.setLayout(raw_layout)\n            layout.addWidget(raw_group, 1)\n'''
new = '''            raw_group, self.wifi_raw_toggle_btn, self.wifi_raw_box = self._make_hidden_details_group(\n                "Technical Details", "Show raw Wi-Fi command output", QTextEdit.WidgetWidth, 150\n            )\n            layout.addWidget(raw_group)\n'''
if old not in text:
    raise SystemExit("Wi-Fi raw block not found")
text = text.replace(old, new, 1)

# Loaded Latency raw JSON -> hidden details.
...
new = '''            raw_group, self.loaded_raw_toggle_btn, self.loaded_raw_box = self._make_hidden_details_group(\n                "Technical Details", "Show ping log / raw LibreSpeed output", QTextEdit.WidgetWidth, 170\n            )\n            layout.addWidget(raw_group)\n'''
if old not in text:
    raise SystemExit("Loaded raw block not found")
text = text.replace(old, new, 1)

# MTU raw output -> hidden details.
old = '''            raw_group = QGroupBox("Raw Ping Output")\n            raw_layout = QVBoxLayout()\n            self.mtu_raw_box = QTextEdit()\n            self.mtu_raw_box.setReadOnly(True)\n            self.mtu_raw_box.setLineWrapMode(QTextEdit.WidgetWidth)\n            raw_layout.addWidget(self.mtu_raw_box)\n            raw_group.setLayout(raw_layout)\n            layout.addWidget(raw_group, 1)\n'''
new = '''            raw_group, self.mtu_raw_toggle_btn, self.mtu_raw_box = self._make_hidden_details_group(\n                "Technical Details", "Show raw ping output", QTextEdit.WidgetWidth, 150\n            )\n            layout.addWidget(raw_group)\n'''
if old not in text:
    raise SystemExit("MTU raw block not found")
text = text.replace(old, new, 1)

# Traceroute raw output -> hidden details below parsed table.
old = '''            self.trace_raw_box = QTextEdit()\n            self.trace_raw_box.setReadOnly(True)\n            self.trace_raw_box.setMinimumHeight(120)\n            self.trace_raw_box.setPlaceholderText("Raw traceroute output will appear here.")\n            layout.addWidget(self.trace_raw_box, 1)\n'''
new = '''            raw_group, self.trace_raw_toggle_btn, self.trace_raw_box = self._make_hidden_details_group(\n                "Technical Details", "Show raw traceroute output", QTextEdit.WidgetWidth, 120\n            )\n            self.trace_raw_box.setPlaceholderText("Raw traceroute output will appear here.")\n            layout.addWidget(raw_group)\n'''
if old not in text:
    raise SystemExit("Traceroute raw block not found")
text = text.replace(old, new, 1)
